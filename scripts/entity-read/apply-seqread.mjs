...
// Robust to cross-agent inconsistencies:
//   - resolves each BOUND entry by canonical_name -> graph_entities.id (NOT the raw entity_id, which some
//     agents recorded as entity_research.id), falling back to a valid entity_id.
//   - reconciles the NEW list across regions + the DB: a name bound in ANY region, or uniquely matching an
//     existing entity by canonical/alias, is recovered (bound), not created.
...
for (const f of readdirSync(dir).filter(f => /^region-\d+\.json$/.test(f)).sort()) {
  ...
  regions.push({ range: meta.range, lm, map });
}
...
console.log(`mentions=${mentions.length} resolved-unique=${inserts.size} recovered=${recovered} new-persons=${newq.size} ambiguous=${amb} non-entity=${non} unmatched=${unmatched} nocid=${nocid}`);
