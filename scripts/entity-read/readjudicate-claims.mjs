...
// paragraph text for the two seed docs
const paraMap = new Map();
for (const doc of [21308, 21310]) for (const r of await queryAll(`SELECT external_para_id pid, text FROM content WHERE doc_id=? AND deleted_at IS NULL AND external_para_id IS NOT NULL`, [doc])) paraMap.set(`${doc}|${r.pid}`, String(r.text).replace(/\s+/g, ' ').trim());
...
2. FAITHFULNESS: flag any claim whose statement asserts a detail (esp. a PLACE, date, or fate) that is NOT supported by its paragraph — e.g. "martyred in Tabríz" where the paragraph says Ṭihrán.
...
for (const e of ents) {
  ...
  const body = claims.map((c, i) => { const p = paraMap.get(`${c.doc_id}|${c.para_id}`) || '(paragraph not found)';
    return `${i}. (${c.relation}) ${c.statement}\n     PARA[${c.para_id}]: ${p.slice(0, 320)}`; }).join('\n');
  let r = {};
