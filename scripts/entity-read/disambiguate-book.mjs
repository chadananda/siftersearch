...
• PLACE and ERA in force. Inherit from the running context; change only when THIS paragraph moves location or time. Trust the text's SOLAR anchors — Naw-Rúz (the spring equinox, ~21 March) and seasons are reliable; count solar years from a known epoch (the Báb's Declaration = May 1844; His martyrdom = July 1850). E.g. "the seventh Naw-Rúz after the Declaration" = spring 1851 (1844 + 7). Treat lunar Hijri (A.H.) dates as approximate. The chapter also fixes the era; give an approximate time (a year or short range), NOT the heading text.
