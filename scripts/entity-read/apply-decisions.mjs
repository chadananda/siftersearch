...
if (WRITE) {
  ...
  for (const d of toApply) {
    ...
    const up = await query(`UPDATE entity_mentions_v2 SET entity_id=?, resolution_basis='reconcile', resolution_conf=? WHERE resolved_as=? AND (entity_id IS NULL OR entity_id=?)`, [eid, d.confidence, d.p.resolved_as, eid]);
    mentions += (up?.changes || 0);
