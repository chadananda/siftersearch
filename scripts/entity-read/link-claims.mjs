...
// entity_claims.entity_id is NOT NULL (schema), and carries STALE literal binds from the old extract-claims bind().
// We can't null them; instead OVERWRITE with the EVIDENCE-based mention entity_id wherever a bound mention matches
// (never literal). Claims with no bound-mention match keep their prior value.
...
const claims = await queryAll(`SELECT id, para_id, semantic_key FROM entity_claims WHERE import_batch IN ('db-v2','gpb-v2')`);
