...
// RESUME: a paragraph is "done" only if it has a NEW-FORMAT hyp (a JSON array of >=1 question) AND a thesis.
// >=1 (not 4): the kernel's v3 generator writes ADAPTIVE arrays sized by the paragraph (1-40) — a stricter
// check here would endlessly regenerate them. The old garbage HyPE (newline-joined, no thesis) still fails →
// gets replaced.
const isNewFormat = (hq, th) => { if (!th) return false; try { const a = JSON.parse(hq); return Array.isArray(a) && a.length >= 1; } catch { return false; } };
