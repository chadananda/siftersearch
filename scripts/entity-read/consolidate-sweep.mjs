...
// strip trailing relational descriptors → the core canonical name (keep the full form as an alias)
const core = (s) => clean(String(s || '').replace(/\s*\((?:[^)]*(?:son|brother|father|sister|wife|nephew|uncle|daughter|mother|of)[^)]*)\)\s*$/i, '').replace(/,\s*(?:son|brother|father|sister|wife|nephew|uncle|daughter|mother)\b.*$/i, '').replace(/,\s*the\b.*$/i, '')).trim() || clean(s);
...
let enrich = 0, create = 0; const toCreate = [];
...
for (const m of merged.values()) {
  ...
  if (id) { // enrich existing
    ...
    if (WRITE) { const p = byId.get(id); let n = {}; try { n = JSON.parse((await queryOne('SELECT research_notes rn FROM entity_research WHERE canonical_name=?', [p.cn]))?.rn || '{}'); } catch {}
      const f2 = Array.isArray(n.facts2) ? n.facts2 : []; if (!f2.some((f) => f.paraId === m.paraId && f.relation === fact.relation)) { f2.push(fact); n.facts2 = f2; await query('UPDATE entity_research SET research_notes=?, updated_at=CURRENT_TIMESTAMP WHERE canonical_name=?', [JSON.stringify(n), p.cn]); } }
  } else { // create
    ...
    if (WRITE && CREATE) {
      ...
      const notes = JSON.stringify({ facts2: [{ ...fact, url: url(m.paraId) }], death, kinship: m.kin, needs_corpus_research: true, created_from: 'person-sweep', roster_source: m.ref });
