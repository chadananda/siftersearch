...
const inName = process.argv[2] || 'namesake-worklist.json';
const outName = inName.replace('worklist', 'decisions').replace(/^.*\//, '');
const work = JSON.parse(readFileSync(`${dir}/${inName}`, 'utf8'));
...
writeFileSync(`${dir}/${outName}`, JSON.stringify(out, null, 1));
let remove = 0, errs = 0;
