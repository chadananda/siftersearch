...
for (const q of QUESTIONS) {
  ...
  for (const spec of MODELS) {
    ...
    try {
      ...
      rows.push({ q: q.slice(0, 28), model: spec, first_ms: first, total_ms: Date.now() - t0, words: out.split(/\s+/).length, links: links.length, bad_links: bad.length });
      console.log(`\n--- ${spec} | ${q}\n${out.slice(0, 700)}`);
...
for (const [m, rs] of Object.entries(by)) {
  ...
  console.log(`${m.padEnd(34)} n=${rs.length} first p50 ${med('first_ms')}ms  total p50 ${med('total_ms')}ms  words p50 ${med('words')}  bad links ${rs.reduce((s, r) => s + r.bad_links, 0)}/${rs.reduce((s, r) => s + r.links, 0)}`);
}
process.exit(0);
