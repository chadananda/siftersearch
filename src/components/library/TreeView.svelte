        ...
        <span class="flex items-center gap-1.5 flex-1 truncate">
          <span class="w-5 h-5 flex items-center justify-center shrink-0 {isExpanded ? 'text-white' : 'text-accent'}">
            {#if religion.name?.toLowerCase().includes('baha')}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="1.699 1.631 19.772 19.847" class="w-[1.125rem] h-[1.125rem]">
            ...
            {:else if religion.symbol}
              <span class="text-[1.125rem] leading-none">{religion.symbol}</span>
            {/if}
          </span>
