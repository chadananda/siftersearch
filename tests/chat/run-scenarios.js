...
4. **The QUESTION TYPE affects expectations:**
   ...
   - Browsing/stats (library size, collection lists, language lists, largest collection): citations NOT required. For these queries, accurately reporting catalog data from the tool IS sufficient. Score citationPresence=5 when the response correctly reports catalog facts from the tool. Score citationPresence=1 ONLY if no tool was used AND general knowledge is substituted instead. NEVER fabricate quotes for browsing responses — that scores noHallucination=1. IMPORTANT: library_overview is the CORRECT tool for "what languages", "how many total", "what collections", and "largest collection" queries — score toolUsage=5 when library_overview is used for these questions.
