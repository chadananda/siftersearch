...
export function memStore(seed = {}) {
  ...
  const arabicAliases = []; // registerArabicAliases calls
  ...
  return {
    saved, hyped, mentions, claims, decisions, decisionBatches, created, bound, unbound, arabicAliases, appliedMarks, merges, conceptClaims, conceptDecisions, lexiconEntries, conceptLinks,
    ...
    saveHype: async (paragraphId, questions, thesis, version) => { hyped.push({ paragraphId, questions, thesis, version }); },
    getParaClaims: async (id) => seed.paraClaims?.[id] || {},   // pid → cited-claim statements (fact-informed HyPE)
