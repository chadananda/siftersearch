...
import { BIO_ROOT, listBioPersons, getBioPerson, bioSearch } from '../lib/bio.js';   // shared bio data layer (also powers /api/v1/people)
...
export default async function graphRoutes(server) {
  ...
  server.get('/bio/search', async (request) => await bioSearch(request.query?.q));
